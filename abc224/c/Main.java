...
public final class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        for (int k = j + 1; k < n; k++) {
          int diffx_1 = points[i].x - points[j].x;
          int diffy_1 = points[i].y - points[j].y;
          int diffx_2 = points[i].x - points[k].x;
          int diffy_2 = points[i].y - points[k].y;
          if (diffy_1 * diffx_2 != diffy_2 * diffx_1) {
            ans++;
          }
        }
      }
