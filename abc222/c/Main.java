...
public final class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Pair[] ranking = new Pair[n * 2];

    Arrays.setAll(ranking, i -> new Pair(i, 0));
    ...
    for (int match = 0; match < m; match++) {
      ...
      for (int pnum = 0; pnum < n; pnum++) {
        int judge = -1;
        if (arr[pnum * 2][match] == 'C' && arr[pnum * 2 + 1][match] == 'P'
            || arr[pnum * 2][match] == 'P' && arr[pnum * 2 + 1][match] == 'G'
            || arr[pnum * 2][match] == 'G' && arr[pnum * 2 + 1][match] == 'C') {
          judge = 0;
        } else if (arr[pnum * 2][match] == 'C' && arr[pnum * 2 + 1][match] == 'G'
            || arr[pnum * 2][match] == 'P' && arr[pnum * 2 + 1][match] == 'C'
            || arr[pnum * 2][match] == 'G' && arr[pnum * 2 + 1][match] == 'P') {
          judge = 1;
        }
        if (judge >= 0) {
          ranking[pnum * 2 + judge].winCnt += 1;
        }
      }
      Arrays.sort(ranking);
    }
  ...
  static class Pair implements Comparable<Pair> {
    ...
    public int compareTo(Pair o) {
      // 並び順カスタマイズする場合変更
      if (this.winCnt == o.winCnt)
        return (this.num - o.num);
      return (int) -(this.winCnt - o.winCnt);
      // if (this.l == o.l)
      // return (this.r - o.r);
