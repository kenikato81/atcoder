...
public final class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < n; i++) {
      int q = fs.nextInt();
      if (q == 1) {
        nums[curr] = new Pair(fs.nextInt(), fs.nextInt());
        curr++;
      } else {
        long cnt = fs.nextInt();
        long print_total = 0;
        // 出力
        while (true) {
          if (nums[out_curr].count >= cnt) {
            nums[out_curr].count -= cnt;
            print_total += nums[out_curr].num * cnt;
            sb.append(print_total + "\n");
            break;
          } else {
            cnt -= nums[out_curr].count;
            print_total += nums[out_curr].num * nums[out_curr].count;
            out_curr++;
          }
        }
      }
