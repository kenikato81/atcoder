...
public final class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = 0; j < n; j++) {
        if (i != j) {
          map.put(s_arr[j], map.getOrDefault(s_arr[j], 0) + 1);
          map.put(t_arr[j], map.getOrDefault(t_arr[j], 0) + 1);
        }
      }
      if (!(map.getOrDefault(s_arr[i], 0) == 0 || map.getOrDefault(t_arr[i], 0) == 0)) {
